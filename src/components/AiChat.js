...
export default function AiChat() {
    ...
    function handleSend(evt) {
        ...
        console.log(settingStore.chatsWithAi)
        setChatInput("");
        evt.preventDefault();
    }
    return (
        <div className="h-full mx-2 my-2 px-1 py-1 flex flex-col pb-10">
            {settingStore.chatsWithAi.map((ele, index) => 
                (<h1 className={`border text-sm w-8/12 rounded-xl px-2 py-2 my-2 ${ele.role==="user"?"ml-auto":"mr-auto "}`} key={Math.random()*10}>{ele.content}</h1>)
            )}
            <form
